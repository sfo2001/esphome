...
def command_prepare_linux(args: ArgsProtocol, config: ConfigType) -> int | None:
    ...
    if CORE.target_platform != PLATFORM_LINUX:
        _LOGGER.error("prepare-linux is only supported for Linux platform")
        return 1
    ...
    service_content = _generate_systemd_service_for_deploy(device_name)
    ...
    ota_cmd = f"esphome run {args.configuration[0]} --device <IP_ADDRESS>"
...
def _generate_systemd_service_for_deploy(device_name: str) -> str:
    """Generate systemd service file content for deployment package."""
    return f"""[Unit]
...
POST_CONFIG_ACTIONS = {
    ...
    "prepare-linux": command_prepare_linux,
}
...
def parse_args(argv):
    ...
    parser_prepare_linux = subparsers.add_parser(
        "prepare-linux",
        help="Generate Linux deployment package with installer script (Linux platform only).",
    )
    parser_prepare_linux.add_argument(
        "configuration", help="Your YAML configuration file.", nargs=1
    )
    parser_prepare_linux.add_argument(
        "--output-dir",
        "-o",
        help="Output directory for deployment package (default: <device-name>-deploy)",
        type=str,
        default=None,
    )
